...
#----------------------------------------------------------------------------------
# Gost 转发面板管理脚本 (v20.3 - Debian/Alpine/Direct)
#
# 作者: Gemini
...
# ... (v20.2 及更早版本日志省略) ...
#
#   - [V20.3] 彻底重构安装流程 (根据用户建议):
#       1. [移除] 删除了所有不可靠的 IP 检测 (移除了 detect_china_ip 函数)。
#       2. [重构] install_gost 函数，在依赖安装后提供 "手动" vs "自动" 选项。
#       3. [手动] 模式会尝试获取最新版本号以提示用户，并在 GitHub 失败时回退。
#       4. [自动] 模式执行标准海外 VPS 下载流程。
#----------------------------------------------------------------------------------
...
# (v20.3) detect_china_ip 函数已移除，不再需要
...
install_gost() {
    ...
    # --- [NEW v20.3 LOGIC START] ---
    local DOWNLOAD_MODE="auto" # Default
    
    print_info "请选择安装模式:"
    echo " 1. 手动安装 (适用于中国大陆或 GitHub 连接不良的 VPS)"
    echo " 2. 自动安装 (适用于海外 VPS)"
    ...
    if [ "$choice" = "1" ]; then
        ...
        print_info "已选择手动安装模式。"
    elif [ "$choice" = "2" ]; then
        DOWNLOAD_MODE="auto"
        print_info "已选择自动安装模式。"
    else
        print_error "无效选项。已取消安装。"
        return
    fi
    # --- [NEW v20.3 LOGIC END] ---
    ...
    if [ $GITHUB_STATUS -ne 0 ] || [ -z "$LATEST_URL" ] || [[ "$LATEST_URL" != *"github.com"* ]]; then
        ...
        if [ "$DOWNLOAD_MODE" = "auto" ]; then
            # 用户选择了 "Auto" 但连接失败
            print_error "自动安装失败。请检查网络或尝试 '手动安装'。"
            return
        else
            # 用户选择了 "Manual"，连接失败是预料之中的
            print_warning "将切换到无版本号的手动上传模式。"
            FILENAME="gost_latest_linux_${ARCH}.tar.gz" # 提示一个示例文件名
            DOWNLOAD_URL="" # 无法获取 URL
        fi
    else
        # (GitHub 连接成功)
    ...
    if [ "$DOWNLOAD_MODE" = "manual" ]; then
        ...
        if [ -n "$DOWNLOAD_URL" ]; then
            # (GitHub 成功，我们知道确切的 URL)
            print_info "-----------------------------------------"
            print_info "请下载以下文件:"
            print_warning "  ${DOWNLOAD_URL}"
            print_info "文件应另存为 (或重命名为):"
            print_warning "  gost-release.tar.gz"
            print_info "-----------------------------------------"
        else
            # (GitHub 失败，我们不知道 URL)
            print_info "请下载适用于 [linux] 和 [${ARCH}] 的 .tar.gz 压缩包。"
            print_info "(例如: ${FILENAME})"
            print_info "下载后，请将文件重命名为:"
            print_warning "  gost-release.tar.gz"
        fi
        
        echo
        print_info "请将重命名后的 (gost-release.tar.gz) 文件上传到本VPS的以下路径:"
        print_warning "  ${FILE_PATH}" # 这是 /tmp/gost-release.tar.gz
        echo
...
config_socks5_auth() {
    ...
    # (v20.3 修复) 对包含特殊字符的 user/pass 进行 URL 编码
    local encoded_user=$(echo -n "$user" | sed -e 's/@/%40/g' -e 's/:/%3A/g' -e 's/\//%2F/g')
    local encoded_pass=$(echo -n "$pass" | sed -e 's/@/%40/g' -e 's/:/%3A/g' -e 's/\//%2F/g')
    
    local CMD_LINE="-L 'socks5://${encoded_user}:${encoded_pass}@:${port}'"
...
config_socks5_chain_tls() {
    ...
    local CMD_LINE="-L 'socks5://${encoded_user}:${encoded_pass}@:${lport}' -F 'relay+tls://${raddr}?insecure=true'"
    echo "${CMD_LINE}" >> "${CONFIG_FILE}"
...
# (v20.2) 新增: SOCKS5 中转 (加密到下一跳 SS)
config_socks5_chain_ss() {
    local lport user pass raddr rport rpass method

    print_info "--- 添加 SOCKS5 中转 (-> SS) ---"
    print_info "此服务将作为 SK5 服务端，并将流量转发到 SS 落地机。"
    
    read -p "请输入 [本地] SOCKS5 监听端口 : " lport
    read -p "请输入 [本地] SOCKS5 用户名 : " user
    read -p "请输入 [本地] SOCKS5 密码 : " pass
    echo
    print_info "--- 请输入远程 SS 落地机信息 ---"
    read -p "请输入 [远程] SS 服务器 IP 地址: " raddr
    read -p "请输入 [远程] SS 服务器 端口 : " rport
    read -p "请输入 [远程] SS 服务器 密码 : " rpass
    
    echo "请选择 [远程] SS 加密方法 (必须与服务器一致):"
    local METHODS=("aes-256-gcm" "chacha20-poly1305")
    PS3="请输入选项 [1-2]: "
    ...
    if [ -z "$lport" ] || [ -z "$raddr" ] || [ -z "$rport" ] || [ -z "$rpass" ]; then
        print_error "本地端口、远程地址、远程端口或远程密码不能为空。"
        return
    fi
    ...
    # (v20.3 修复) 对所有 user/pass 进行 URL 编码，防止特殊字符
    local encoded_user=$(echo -n "$user" | sed -e 's/@/%40/g' -e 's/:/%3A/g' -e 's/\//%2F/g')
    local encoded_pass=$(echo -n "$pass" | sed -e 's/@/%40/g' -e 's/:/%3A/g' -e 's/\//%2F/g')
    local encoded_rpass=$(echo -n "$rpass" | sed -e 's/@/%40/g' -e 's/:/%3A/g' -e 's/\//%2F/g')
    
    # -F 'ss://[method]:[ss_pass]@[ss_server_ip]:[ss_port]'
    local CMD_LINE="-L 'socks5://${encoded_user}:${encoded_pass}@:${lport}' -F 'ss://${method}:${encoded_rpass}@${raddr}:${rport}'"
    echo "${CMD_LINE}" >> "${CONFIG_FILE}"

    print_success "SOCKS5 链式转发 (:${lport} -> ss://${raddr}:${rport}) 已添加。"
    ask_for_restart
}
...
# (v20.1) 配置 - SS 落地服务端 (带随机密码)
config_ss_listener() {
    local lport pass method

    print_info "--- 添加 Shadowsocks (SS) 落地服务端 ---"
    print_info "此服务将作为 SS 终端，接收来自中转 VPS 的加密流量。"
    
    read -p "请输入 SS 监听端口 : " lport
    
    if [ -z "$lport" ]; then
        print_error "监听端口不能为空。"
        return
    fi
    
    # (v20.3 修复) 
    # openssl rand -base64 12 会产生 '+' 和 '/'，它们在 URL 中是非法的
    # 我们使用 'openssl rand -hex 12' 来生成一个 24 字符的纯净十六进制密码
    pass=$(openssl rand -hex 12)
    if [ -z "$pass" ]; then
        print_error "随机密码生成失败！"
        return
    fi
    
    echo "请选择 SS 加密方法 (推荐: 1):"
    # (v20.1) 按用户要求只提供两种
    local METHODS=("aes-256-gcm" "chacha20-poly1305")
    PS3="请输入选项 [1-2]: "
    select method in "${METHODS[@]}"; do
        if [ -n "$method" ]; then
            print_info "已选择加密方法: $method"
            break
        else
            print_error "无效选项 $REPLY"
        fi
    ...
    print_warning "请确保在防火墙 (安全组) 中放行 ${lport} 的 TCP 和 UDP 端口。"
    init_config_file
    
    local CMD_LINE="-L 'ss://${method}:${pass}@:${lport}'"
    echo "${CMD_LINE}" >> "${CONFIG_FILE}"

    print_success "SS 落地服务端 (:${lport}) 已添加到 ${CONFIG_FILE}"
    echo "-----------------------------------------"
    print_warning "您的 SS 配置信息如下 (请立即保存):"
    print_info    "  地址:   (你的海外VPS IP)"
    print_info    "  端口:   ${lport}"
    print_warning "  密码:   ${pass}"
...
manage_config() {
    while true; do
        ...
        echo
        echo "--- 添加新服务 (隧道 - 中转机/国内VPS) ---"
        echo -e " 5. ${GREEN}添加: SOCKS5 中转 (-> TLS)${NC}"
        echo -e " 6. ${GREEN}添加: SOCKS5 中转 (-> SS)${NC}"
        echo
        echo "--- 添加新服务 (隧道 - 落地机/海外VPS) ---"
        echo -e " 7. ${GREEN}添加: TLS 隧道服务端${NC}"
        echo -e " 8. ${GREEN}添加: Shadowsocks (SS) 落地服务端${NC}"
        echo
        echo "--- 管理配置 ---"
        echo -e " 9. ${YELLOW}删除一个指定配置${NC}"
        echo
        echo " 0. 返回主菜单"
        echo
