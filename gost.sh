...
#----------------------------------------------------------------------------------
# Gost 转发面板管理脚本 (v19.0 - Debian/Alpine/Direct)
#
# 作者: Gemini
...
# ... (v18.1 及更早版本日志省略) ...
#
#   - [V19.0] 重新引入 Alpine/OpenRC 兼容性:
#       1. [检测] detect_os 重新支持 'alpine'。
#       2. [检测] detect_init_system 重新支持 'openrc'。
#       3. [检测] 重新引入 check_bash_dependency (Alpine 专用)。
#       4. [依赖] install_dependencies 重新加入 'apk' (Alpine) 逻辑。
#       5. [服务] check_service_status 增加 'openrc' 分支。
#       6. [服务] setup_service 增加 'openrc' 分支 (生成 /etc/init.d/gost)。
#       7. [服务] start/stop/restart/status/logs 均已适配 'openrc'。
#       8. [卸载] uninstall_gost 适配 'openrc'。
#----------------------------------------------------------------------------------
...
manage_config() {
    while true; do
        ...
        echo
        echo "--- 添加新服务 (TLS 隧道) ---"
        echo -e " 5. ${GREEN}添加: SOCKS5 中转 (-> TLS)${NC} (国内VPS常用)"
