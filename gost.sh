...
#----------------------------------------------------------------------------------
# Gost 转发面板管理脚本 (v20.1 - Debian/Alpine/Direct)
#
# 作者: Gemini
...
#
#   - [V20.1] 添加 SS 落地服务功能:
#       1. [菜单] 新增 "7. 添加: Shadowsocks (SS) 落地服务端"。
#       2. [SS] SS 密码自动随机生成。
#       3. [SS] SS 加密方式精简为 aes-256-gcm 和 chacha20-poly1305。
#----------------------------------------------------------------------------------
...
manage_config() {
    while true; do
        ...
        echo
        echo "--- 添加新服务 (TLS / SS 隧道) ---"
        echo -e " 5. ${GREEN}添加: SOCKS5 中转 (-> TLS)${NC} (国内VPS)"
        echo -e " 6. ${GREEN}添加: TLS 隧道服务端${NC} (海外VPS)"
        echo -e " 7. ${GREEN}添加: Shadowsocks (SS) 落地服务端${NC} (海外VPS)"
        echo
        echo "--- 管理配置 ---"
        echo -e " 8. ${YELLOW}删除一个指定配置${NC}"
        echo
        echo " 0. 返回主菜单"
        echo
